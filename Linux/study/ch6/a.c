    ...
    do {
        ...
        if(FD_ISSET(ssock, &readfd)) { 			/* 읽기가 가능한 소켓이 서버 소켓인 경우 */
            ...
            if(csock < 0) {
                ...
                client_ip[client_index] = (char*)malloc(strlen(mesg)+1);
                strcpy(client_ip[client_index], mesg);
                client_index++;
                continue;
            }
        ...
        for(start_index = 0; start_index < client_index; start_index++) {
            ...
            if(FD_ISSET(client_fd[start_index], &readfd)) {
                ...
                if((n = read(client_fd[start_index], mesg, sizeof(mesg))) > 0) {
                    printf("Received data (%s): %s", client_ip[start_index], mesg);
                    write(client_fd[start_index], mesg, n);
                    if(strncmp(mesg, "q", 1) == 0) {
                        printf("Client is disconnected : %s\n", client_ip[start_index]);
                        close(client_fd[start_index]); 	/* 클라이언트 소켓을 닫는다. */
                        FD_CLR(client_fd[start_index], &readfd);    /* 클라이언트 소켓을 지운다. */
                        client_index--;
                    }
                }
            }
        }
    } while(strncmp(mesg, "q", 1) || client_index != 0);
